...
from utils.llm_service import analyze_all_clauses_batch, reformat_contract_text
from utils.pdf_highlighter import highlight_pdf, create_highlighted_pdf_with_text
from utils.report_service import generate_html_report
...
async def analyze_document(
    ...
    try:
        ...
        # [STEP 1] OCR 수행
        raw_text_ocr = await run_ocr(file, lang_code=lang)
        if not raw_text_ocr or len(raw_text_ocr) < 10:
            raise HTTPException(status_code=400, detail="텍스트를 추출할 수 없습니다.")
        
        # 줄바꿈 정규화 (\r\n -> \n)
        raw_text_normalized = raw_text_ocr.replace('\r\n', '\n').replace('\r', '\n')

        # [NEW STEP] AI 기반 텍스트 레이아웃 정제
        raw_text = reformat_contract_text(raw_text_normalized)

        # [STEP 2] 조항 분리
        clauses = split_clauses(raw_text)
        if not clauses: clauses = [raw_text]

        # [STEP 3] 법률 검색 (clauses의 'text' 필드 추출하여 전달)
        related_laws_per_clause = [retrieve_related_laws(c["text"]) for c in clauses]

        # [STEP 4] GPT 분석 (clauses 리스트 전체 전달 - 내부에서 위치 정보 처리)
        ...
        for res in analysis_results:
            if res.get('severity') in ['CRITICAL', 'WARNING', 'DISADVANTAGE']:
                risk_count += 1
...
async def download_report():
    ...
    try:
        ...
        filename = last_analysis.get("filename", "contract")
        
        # HTML 리포트 생성
        html_content = generate_html_report(results, raw_text, filename)
        
        # HTML 파일명 생성 (URL 인코딩)
        from urllib.parse import quote
        base_name = filename.rsplit('.', 1)[0] if '.' in filename else filename
        output_filename = f"analysis_report_{base_name}.html"

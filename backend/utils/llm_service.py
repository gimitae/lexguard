...
def analyze_all_clauses_batch(clauses: list, related_laws_per_clause: list):
    ...
    for i, clause_data in enumerate(clauses):
        # clause_data는 {"text": "...", "start": 0, "end": 10} 형태
        clause_text = clause_data["text"]
        start_pos = clause_data["start"]
        end_pos = clause_data["end"]
        ...
        try:
            ...
            analysis = analyze_clause(clause_text, related_laws, context=full_context)

            analysis["clause_number"] = i + 1
            analysis["original_text"] = clause_text[:100]
            analysis["start"] = start_pos
            analysis["end"] = end_pos
...
def reformat_contract_text(raw_text: str):
    """
    OCR로 추출된 투박한 텍스트를 AI를 통해 가독성 좋은 계약서 형식으로 재구성.
...
## 지침:
1. 모든 정보는 가능한 한 컴팩트하게 유지하라. 항목명(예: '1. 근로개시일 :')과 그 내용물은 반드시 **같은 줄**에 배치하라.
2. 조항의 시작(예: 제1조, 제2조)이나 번호가 매겨진 항목(예: 1., 2.)만 새로운 줄에서 시작하라.
3. 조항(항목) 내에서나 조항 사이에서 불필요한 빈 줄(Blank Line)을 절대로 추가하지 마라.
4. OCR 오류로 인해 단어가 쪼개져 있다면(예: '2026 년' -> '2026년') 자연스럽게 합쳐라.
5. 결과물은 마크다운 형식이 아닌 순수 텍스트(Plain Text)로 반환하라.

## 출력 예시:
1. 근로개시일 : 2026년 03월 01일부터
2. 근무장소 : ㈜바른계약 울산데이터센터 (울산광역시 중구 이예로30 데이터 이노시스 3F)
3. 업무의 내용 : 백엔드 관리사업 보조 및 이하 상사의 명령에 따른 모든 부가적 업무
...
"""
    ...
    try:
        response = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "너는 법률 문서의 가독성을 최적화하는 전문 편집 도우미이다."},
                {"role": "user", "content": prompt}
            ],
            temperature=0
        )
